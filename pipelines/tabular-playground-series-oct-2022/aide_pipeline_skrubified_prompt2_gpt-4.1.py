...
def combine_preds(pred_A, pred_B):
    # pred_A and pred_B are (n,2) arrays from predict_proba, take [:,1]
    import numpy as np
    import pandas as pd
    return pd.DataFrame({
...
# Prepare submission
submission = pd.concat([test_df["id"], pd.DataFrame(test_preds, columns=["team_A_scoring_within_10sec", "team_B_scoring_within_10sec"])], axis=1)
